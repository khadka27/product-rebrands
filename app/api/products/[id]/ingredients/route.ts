...
import { addIngredient, getIngredientsByProductId, deleteIngredientsByProductId } from "@/lib/models/ingredient"
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const ingredients = await getIngredientsByProductId(Number.parseInt(params.id))
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const deleted = await deleteIngredientsByProductId(Number.parseInt(params.id))
